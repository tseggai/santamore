...
| Strava API application (client id + secret), webhook verify token | `.env.example` → `STRAVA_*`; register at strava.com/settings/api with our host as the callback domain, then "Register webhook" on `/admin/izazovi` | Partner perks (docs/STRAVA.md) |
