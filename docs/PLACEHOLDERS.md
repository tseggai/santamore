...
| Strava API application (client id + secret), webhook verify token — needs an organisation-controlled Strava account **with a paid subscription**; new apps are capped at 1 athlete, then 10 self-service, then Developer Program review with screenshots | `.env.example` → `STRAVA_*`; register at strava.com/settings/api with our host as the callback domain, then "Register webhook" on `/admin/dogadjaji` (Strava webhook panel) | Partner perks (docs/STRAVA.md); submit for review well before the first public challenge |
...
| Partner cafe name, reward wording, daily capacity and PIN | `/admin/dogadjaji` (Strava webhook panel) form | First live challenge |
