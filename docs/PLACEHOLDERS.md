...
| Board, grants-committee and team names + photos with consent | Team: Admin → Members → a member's panel ("Show on the About us page"); board/committee wording: Admin → Content → Pages → About us | `/o-nama` team section |
...
| Strava API application (client id + secret), webhook verify token — needs an organisation-controlled Strava account **with a paid subscription**; new apps are capped at 1 athlete, then 10 self-service, then Developer Program review with screenshots | `.env.example` → `STRAVA_*`; register at strava.com/settings/api with our host as the callback domain, then "Register webhook" on `/admin/izazovi` | Partner perks (docs/STRAVA.md); submit for review well before the first public challenge |
