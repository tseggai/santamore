...
| BIC | env var above — brief §15.4 asks for IBAN **and** BIC; BIC was untracked until now | EPC QR payload (optional under v002), impressum |
...
| `docs/vendor/epc-qr.md` — the EPC069-12 spec | `docs/vendor/` is still empty; the cloud session cannot reach the EPC site (egress-blocked). `lib/epc-qr.ts` is built from the brief §8 block + published EPC069-12 rules, with our reference in the **unstructured** remittance line (the structured line legally requires ISO 11649 "RF…", which `SM-…` is not — brief deviation, flagged) | Verify the builder against the saved spec **and test the QR in ≥2 real EU banking apps** before launch |
...
| Russian donate/admin/email strings — native review | `messages/ru.json` (new `donate.*`, `email.*`, `admin.*` keys, drafted) | Same `_review` flag as the rest of ru.json |
