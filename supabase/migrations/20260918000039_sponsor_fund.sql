-- ============================================================================
-- Santamore — which fund a sponsorship feeds
-- Apply AFTER 20260918000038_years_recorded.sql. Re-runnable.
--
-- By default sponsorship cash is the Operations Fund (brief §11). In 2025
-- every euro, sponsors' included, was handed to the beneficiaries with no
-- operating cost held back (owner, 2026-09-18), so a deal can name the
-- Impact Fund instead; it then counts as raised, not as operations.
-- ============================================================================

alter table public.sponsors add column if not exists fund text not null default 'operations';
alter table public.sponsors drop constraint if exists sponsors_fund_check;
alter table public.sponsors add constraint sponsors_fund_check check (fund in ('operations', 'impact'));

-- 2025: everything went to the beneficiaries.
update public.sponsors s
 ...
 where s.id = s2.id
   and coalesce(s2.year, extract(year from coalesce(e.starts_at, c.starts_at))::int) = 2025
   and s.fund = 'operations';
...
select
  coalesce((select sum(s.amount_cents) from public.sponsors s
            where s.status in ('signed', 'active')
              and s.is_in_kind = false
              and s.fund = 'operations'), 0)
...
sponsor_cash as (
  -- a deal names its fund: operations (the default) or impact, when the
  -- money went to beneficiaries as donations do
  select year,
         sum(amount_cents) filter (where fund = 'operations') as cents,
         sum(amount_cents) filter (where fund = 'impact')     as impact_cents
    from sponsor_rows
...
select
  y.year,
  coalesce(mi.cents, 0) + coalesce(ai.cents, 0) + coalesce(sc.impact_cents, 0) as received_cents,
