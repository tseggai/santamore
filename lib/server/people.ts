...
export async function loadPeople() {
  ...
  const [{ data: members }, { data: pages }, { data: registrations }, { data: teams }, { data: events }, { data: causeRows }, { data: teamRows }] =
    await Promise.all([
      ...
      supabase.from("fundraisers").select("id, user_id, slug, title, status, goal_cents, campaign_id, team_id").limit(5000),
      supabase
        .from("registrations")
        ...
        .limit(10_000),
      supabase.from("teams").select("id, captain_id, name, slug, campaign_id, event_id").limit(5000),
  ...
  const memberPages: MemberPage[] = ((pages ?? []) as { id: string; user_id: string; slug: string; title: string; status: "draft" | "active" | "hidden"; goal_cents: number | null; campaign_id: string | null; team_id: string | null }[]).map((page) => ({
    ...page,
  ...
  const memberTeams: MemberTeam[] = ((teams ?? []) as { id: string; captain_id: string | null; name: string; slug: string; campaign_id: string | null; event_id: string }[]).map((team) => ({
    ...team,
